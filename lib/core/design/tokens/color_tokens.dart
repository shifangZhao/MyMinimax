/// Design tokens ported from shadcn/ui v4 themes.ts.
/// All colors in OKLCH for perceptually uniform lightness.
/// Base colors define the full semantic palette; accent themes override
/// only primary/secondary/chart/sidebar on top of a chosen base.
class ColorTokens {
  ColorTokens._();

  static const baseColorNames = [
    'neutral', 'stone', 'zinc', 'mauve', 'olive', 'mist', 'taupe',
  ];

  static const accentThemeNames = [
    'amber', 'blue', 'cyan', 'emerald', 'fuchsia', 'green', 'indigo',
    'lime', 'orange', 'pink', 'purple', 'red', 'rose', 'sky', 'teal',
    'violet', 'yellow',
  ];

  /// Build a complete set of CSS custom properties for `:root` and `.dark`.
  /// [baseColor] is one of [baseColorNames]; [accentTheme] is optional.
  static String buildCSSVars({
    required String baseColor,
    String? accentTheme,
  }) {
    final base = _baseColors[baseColor] ?? _baseColors['neutral']!;
    final accent =
        accentTheme != null ? _accentThemes[accentTheme] : null;

    final mergedLight = Map<String, String>.from(base.light);
    final mergedDark = Map<String, String>.from(base.dark);
    if (accent != null) {
      mergedLight.addAll(accent.light);
      mergedDark.addAll(accent.dark);
    }

    final buf = StringBuffer();
    buf.writeln(':root {');
    for (final e in mergedLight.entries) {
      buf.writeln('  --${e.key}: ${e.value};');
    }
    buf.writeln('}');
    buf.writeln('.dark {');
    for (final e in mergedDark.entries) {
      buf.writeln('  --${e.key}: ${e.value};');
    }
    buf.writeln('}');
    return buf.toString();
  ...
  /// Build full CSS including radius scale and semantic Tailwind bridge.
  /// Call this to inject the complete design token stylesheet.
  static String buildFullCSS({
    required String baseColor,
    String? accentTheme,
    String radius = '0.625rem',
    String fontSans = 'Inter',
    String? fontHeading,
    String fontMono = 'JetBrains Mono',
  }) {
    final buf = StringBuffer();
    ...
    // ── Color tokens ──
    buf.write(buildCSSVars(baseColor: baseColor, accentTheme: accentTheme));
    ...
    buf.writeln('  --code-highlight: var(--accent);');
    buf.writeln('  --code-number: var(--primary);');
  ...
  // ═══════════════════════════════════════════════════════════════
  // Base colors — full semantic palette (light + dark)
  // ═══════════════════════════════════════════════════════════════

  static final _baseColors = <String, _ColorSet>{
    'neutral': const _ColorSet(
      light: {
  ...
  // ═══════════════════════════════════════════════════════════════
  // Accent themes — override only primary/secondary/chart/sidebar
  // ═══════════════════════════════════════════════════════════════

  static final _accentThemes = <String, _ColorSet>{
    'amber': const _ColorSet(
      light: {
...
class _ColorSet {
  const _ColorSet({required this.light, required this.dark});
